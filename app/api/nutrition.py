...
def set_active_nutrition(
    ...
    try:
        ...
        # 3) meals ekle (aynı meal_type 2 kere gelmesin diye unique var)
        for meal in payload.meals:
            cur.execute(
                """
                INSERT INTO nutrition_meals (nutrition_program_id, meal_type, content, order_index, created_at, updated_at)
                VALUES (%s, %s, %s, %s, NOW(), NOW())
                """,
                (program_id, meal.meal_type, meal.content, meal.order_index),
            )
