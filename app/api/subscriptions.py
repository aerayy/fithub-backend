...
def confirm_subscription(
    ...
    # Try idempotency check if external_subscription_id or subscription_ref column exists
    ...
    if has_external_id_column:
        ...
        # No idempotency column exists, skip check (will rely on IntegrityError if duplicate)
        print(f"subscription_confirm: No external_subscription_id or subscription_ref column found, skipping idempotency check")
    ...
    if existing_subscription:
        ...
        return SubscriptionConfirmResponse(
            ...
            subscription={
                ...
                "subscription_ref": subscription_ref,  # Return the provided ref even if not stored
    ...
    try:
        ...
        values = [client_user_id, coach_user_id, package_id, plan_name, "active", now, started_at, ends_at, now, now]
        placeholders = ["%s"] * len(values)
        
        # Add external_subscription_id or subscription_ref if column exists
        if has_external_id_column:
        ...
        # Try to find existing subscription if idempotency column exists
        if has_external_id_column:
            cur.execute(
                """
                SELECT id, client_user_id, coach_user_id, package_id, plan_name, status, started_at, ends_at, purchased_at
